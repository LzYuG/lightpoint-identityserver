...
namespace LightPoint.IdentityServer.Blazor.Pages.IdentityServerBusiness.Accounts
{
    partial class ForgetPassword
    {
        ...
        private async Task ResetPasswordSubmit()
        {
            if(string.IsNullOrWhiteSpace(ResetPasswordModel.NewPassword) || ResetPasswordModel.NewPassword != ResetPasswordModel.NewPasswordConfirm)
            {
                await MessageService!.Error("两次输入的密码不一致");
                return;
            }
            // 手机号
            if(TabActiveKey == "0")
            {
                var validateResult = await ValidationCodeCreater!.ValidateValidationCode("ForgetPasswordUsePhone", ResetPasswordModel.PhoneNumber!, ResetPasswordModel.ShortMessageCode!);
                if (validateResult)
                {
                    var user = await ApplicationUserService!.GetApiBoAsync(x => x.PhoneNumber == ResetPasswordModel.PhoneNumber! && x.PhoneNumberConfirmed);
                    if(user != null)
                    {
                        var resetRes = await ApplicationUserService.ResetPassword(this.CurrentSystemAccountConfig, user.UserName!, ResetPasswordModel.NewPassword);
                        if (resetRes.IsSuccess)
                        {
                            await MessageService!.Success("密码已重置");
                            await JSRuntime!.InvokeVoidAsync("history.back");
                            return;
                        }
                        else
                        {
                            await MessageService!.Error(resetRes.Message);
                        }
                    }
                    else
                    {
                        await MessageService!.Error("服务异常");
                        return;
                    }
                }
                else
                {
                    await MessageService!.Error("验证码校验失败");
                }
            }
            // 邮箱
            else if (TabActiveKey == "1")
            {
                var validateResult = await ValidationCodeCreater!.ValidateValidationCode("ForgetPasswordUseEmail", ResetPasswordModel.Email!, ResetPasswordModel.EmailCode!);
                if (validateResult)
                {
                    var user = await ApplicationUserService!.GetApiBoAsync(x => x.Email == ResetPasswordModel.Email! && x.EmailConfirmed);
                    if (user != null)
                    {
                        var resetRes = await ApplicationUserService.ResetPassword(this.CurrentSystemAccountConfig, user.UserName!, ResetPasswordModel.NewPassword);
                        if (resetRes.IsSuccess)
        ...
        {
            if (string.IsNullOrWhiteSpace(ResetPasswordModel.PhoneNumber))
            {
                await MessageService!.Error("请先输入手机号码");
                return;
            }
            if (!await ApplicationUserService!.HasBoAsync(x => x.PhoneNumber == ResetPasswordModel.PhoneNumber! && x.PhoneNumberConfirmed))
            {
                await MessageService!.Error("手机号码还未注册账号");
                return;
            }

            await SendValidationCodeShortMessage("ForgetPasswordUsePhone", "用于该用户忘记密码时进行重置", ResetPasswordModel.PhoneNumber!);
        }
        ...
        public async Task SendResetPasswordByEmailValidationCodeEmail()
        {
            ...
            if (!await ApplicationUserService!.HasBoAsync(x => x.Email == ResetPasswordModel.Email && x.EmailConfirmed))
            {
                await MessageService!.Error("邮箱还未注册账号");
                return;
            }
            await SendValidationCodeEmail("ForgetPasswordUseEmail", $"您正在重置密码", "用于该用户忘记密码时进行重置", ResetPasswordModel.Email!);
        }
